...
| 列 | 内容 |
|---|---|
| 历史记录 | 每次生成都留档，卡片上显示**生成时间**（今天 / 昨天 / 前天 + 时:分）与**总耗时**（如「1 分 23 秒」）；点「恢复」把当时的参数、**参考图**、输出视频一并还原 |
| 生成设置 | 顶部可切换**工作流**（见下）；参考图（可**从剪切板粘贴 / 上传 / 直接拖进来**）、提示词、分辨率、时长、步数、种子；「重置 / 开始生成」在面板头部，不用滚到底 |
| 输出视频 | 视频预览 + 进度 + 下载，**下面就是抠图模块** |
| 序列帧整理 | 取帧间隔 / 跳过开头 → 拆分 → 逐帧取舍 → 动画预览 → **框选裁剪** → 导出 PNG ZIP / 发送到 Eagle |

**可切换两套工作流**

界面内置两份节点图，在「生成设置」顶部下拉里切换，选择会记住：

| | 说明 |
|---|---|
| **T8** | 仓库自带的自研节点图（需要 `comfyui-minimax-h3-audio-T8` 那套节点） |
| **官方** | 原生 MiniMax H3 节点图（`MiniMaxH3ImageToVideo` 等），固定 512×512 |

选「官方」时，它没有对应节点的控件（画面比例 / 分辨率 / 音频模式 / CRF）会**置灰并说明原因**，而不是静默忽略你的改动。
